...
class Optimizer:
    ...
    @classmethod
    def set_class_variables(cls, target=None, obj_fid=1e-4, dim_0=None, dim_1=None, layers=None):
        cls.OBJ_FIDELITY = obj_fid
        cls.SINGLE_DIM_0 = dim_0
        cls.SINGLE_DIM_1 = dim_1
        cls.TARGET_GATE = target
        cls.MAX_NUM_LAYERS = layers if layers is not None else (
            2 * dim_0 * dim_1 if dim_0 is not None and dim_1 is not None else None
        )
        cls.X_SOLUTION = []
        cls.FUN_SOLUTION = []
