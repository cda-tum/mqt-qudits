...
from mqt.qudits.compiler.onedit.randomized_benchmarking.bench_suite import generate_clifford_group, get_h_gate, \
    get_package_data_path, get_s_gate, load_clifford_group_from_file, matrix_hash, save_clifford_group_to_file
...
class TestCliffordGroupGeneration(unittest.TestCase):
    ...
    def test_matrix_hash(self):
        ...
        self.assertEqual(matrix_hash(matrix1), matrix_hash(matrix2))
        self.assertNotEqual(matrix_hash(matrix1), matrix_hash(matrix3))
    ...
    def test_generate_clifford_group(self):
        ...
        self.assertTrue(np.allclose(clifford_group['h'], get_h_gate(2)))
        self.assertTrue(np.allclose(clifford_group['s'], get_s_gate(2)))

        # Check if the group has the expected number of elements
        # For qubit (d=2) and max_length=2, we expect 6 unique elements
        self.assertEqual(len(clifford_group), 6)

    def test_get_package_data_path(self):
        filename = 'test_file.pkl'
        path = get_package_data_path(filename)
        self.assertTrue(os.path.dirname(path).endswith('data'))
        self.assertTrue(os.path.basename(path) == filename)
    ...
    def test_save_and_load_clifford_group(self):
        ...
        try:
            ...
            self.assertEqual(len(clifford_group), len(loaded_group))
